...
\date[April 2026]{NE 511 Project Proposal\\Multiphysics of Nuclear Reactors\\TODO: presentation date}
...
\begin{frame}[allowframebreaks]
    ...
    \begin{gather}
        ...
        \mathcal{H}T(\vec{r}) = \mathcal{Q}(T)\int\limits_{4\pi}d\hat{\Omega} \psi(\vec{r}, \hat{\Omega}, E, t),\tag{Heat Transfer}
    \end{gather}
    ...
    \begin{subequations}
        \begin{gather}
            \mathcal{L}(T)\psi = \left[ \frac{\partial}{\partial t} + \hat{\Omega} \cdot \nabla  + \Sigma_t(T)  \right]\psi\\
            \mathcal{S}(T)\psi =  
            \begin{multlined}[t]
                \frac{\chi(E)}{4\pi} \int\limits_0^\infty dE^\prime  \; \nu\Sigma_f(E^\prime, T) \int\limits_{4\pi}d\hat{\Omega} \psi(\vec{r}, \hat{\Omega}, E^\prime, t) \\+ \int\limits_0^\infty dE^\prime\int\limits_{4\pi}d \Omega^\prime \; \Sigma_s(\hat{\Omega}^\prime \cdot \hat{\Omega}, E^\prime \rightarrow E, T) \psi(\hat{\Omega}^\prime, E^\prime)
            \end{multlined}\\
            \Sigma_i = \Sigma_{i, 0} + \Sigma_{i, 1, d} \left( \sqrt{T + \beta_{fb} W_f f(z)} - \sqrt{T_{d, 0}} \right) + \Sigma_{i, 1, b}\left(T - T_0 \right) \\
            \mathcal{P}_kC_k = \frac{1}{4\pi}\lambda_k \chi_k^{del}(\vec{r}, E, t)C_k(\vec{r}, t) \\
            \mathcal{Q}(T)\int\limits_{4\pi}d\hat{\Omega} \psi(\vec{r}, \hat{\Omega}, E, t) = \int\limits_{0}^\infty dE \; W_f(E)\Sigma_f(E, T)\int\limits_{4\pi}d\hat{\Omega} \psi(\vec{r}, \hat{\Omega}, E, t)\\
            \mathcal{H}T = \left[ \frac{\partial}{\partial t} + \dot{m}c_p\nabla -k_b\Delta  \right]T(\vec{r})
        \end{gather}
    \end{subequations}
    \normalsize
\end{frame}
...
\begin{frame}{General Coupling Strategy}
    \begin{alertblock}{Coupling HT to High-Order Transport is needlessly expensive}
...
\begin{frame}{Coupling with Low-Order Operators}
    ...
    \begin{minipage}{0.39\linewidth}
        \begin{itemize}
            \item Implicit HT uses $H\delta T \approx \mathcal{H}\delta T$ to compute cross sections
            \item Coupling is contained in LO/Approximate system
            \item Requires few solutions of NT and HT equations
            \item Introduces \alert{no approximation} in final solution
        \end{itemize}
...
\begin{frame}{CMFD Coupling Methods~\cite{adamowicz2023nilo}}
...
\begin{frame}{CMFD Coupling Methods -- X-CMFD}
    ...
    \begin{tikzpicture}[overlay,remember picture,black,>=Stealth, nodes={font=\footnotesize}]
        \draw (inner)  ++ (0, -0.30) node[below] {{Solved by "inner iterations"}};
    \end{tikzpicture}
...
\begin{frame}{CMFD Coupling Methods -- NILO-CMFD}
...
\begin{frame}{Multigroup Low-Order Quasidiffusion}
    % Begin with the high-order multigroup NTE in 1D slab geometry,

    % \begin{multline}
    %     \left[\frac{1}{v_g}\frac{\partial}{\partial t} + \mu \frac{d}{dx}+ \Sigma_t \right]\psi_{g} = \frac{1}{2} \sum_{g^\prime} \left(\Sigma_{s, g} + \nu\Sigma_{f, g} \right)\phi_{g^\prime}\\
    %     + q_{del, g} + q_{g}, \qquad g=1,\dots, N_g
    % \end{multline}
    ...
    \begin{subequations}
        ...
        \begin{gather}
            ...
            J_g(0, t) = B_{0, g}\phi_g(0, t), \quad J_g(X, t) = B_{X, g}\phi_g(X, t)
        \end{gather}
    \end{subequations}
...
\begin{frame}{General Project Goals}
    ...
    \begin{itemize}
        ...
        \item Consider delayed neutrons (not included in~\cite{tamang2014multilevel})
...
\begin{frame}{Time, spatial, and angular discretization of ELOT problem}
    ...
    My implementation in Python, but can be scaled up to 2/3D easily using tools such as MFEM~\cite{anderson2021mfem}
